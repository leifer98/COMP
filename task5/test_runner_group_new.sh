...
# Directories
input_dir="tests/group_new/input"
expected_dir="tests/group_new/expected"
output_dir="tests/group_new/output"
ll_dir="tests/group_new/lls"
# Directory .out
program="./hw5"
...
for input_file in "$input_dir"/*.in; do
    ...
    tmp_file1="$ll_dir/$(basename "$input_file").ll"
    touch "$tmp_file1"
    "$program" < "$input_file" > "$tmp_file1" 2>&1
