...
# Directories
input_dir="tests/group/testsHW5"
expected_dir="tests/group/testsHW5"
output_dir="tests/group/testsHW5/output"
program="./hw5"
...
for input_file in "$input_dir"/*.in; do
    ...
    output_llvm_file="$output_dir/$test_name.ll"
    output_file="$output_dir/$test_name.out"

    # Run the program and redirect output
    "$program" < "$input_file" > "$output_llvm_file" 2>&1

    # Run lli and redirect output
    "lli" < "$output_llvm_file" > "$output_file" 2>&1
